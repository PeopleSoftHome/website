...
本目录汇总前端门户中可跨项目复用的模块，已按子目录物理迁移完成。
业务代码统一从 `@/shared/*` 导入；旧路径（`@/api/client`、`@/composables/*`、`@/utils/date` 等）保留兼容性 re-export，但新代码应避免使用。

## 子目录

| 目录 | 说明 | 代表模块 |
|------|------|----------|
| `api/` | HTTP 客户端与统一响应处理 | `client.ts`、`baseUrl.ts` |
| `cms/` | CMS 数据获取与 fallback 注册表 | `useCmsData.ts` |
| `composables/` | 通用 UI / 状态 / 搜索 / 主题 Composables | `useTheme`、`useSearch`、`useSiteConfig`、`useNavigation`、`useFocusTrap`、`useScrollLock` |
