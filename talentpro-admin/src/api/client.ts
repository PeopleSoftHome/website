...
client.interceptors.response.use(
  ...
  async (err: AxiosError<ApiErrorBody>) => {
    ...
    const error = new Error(message) as Error & {
      response?: AxiosError<ApiErrorBody>['response'];
      status?: number;
    };
 ...
 */
export function normalizePaginationResponse<T = unknown>(res: unknown): NormalizedPage<T> {
  if (!res) return { items: [], total: 0 };
  const body = res as { data?: unknown; meta?: { total?: number } };
  const data = body.data;
  if (Array.isArray(data)) {
    return { items: data as T[], total: body.meta?.total ?? data.length };
  }
  if (data && typeof data === 'object') {
    const page = data as { items?: T[]; data?: T[]; total?: number; meta?: { total?: number } };
    return {
      items: page.items || page.data || [],
      total: page.total ?? page.meta?.total ?? 0,
    };
  }
