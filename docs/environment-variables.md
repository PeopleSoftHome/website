...
| `ALIPAY_APP_ID` / `ALIPAY_PRIVATE_KEY` / `ALIPAY_PUBLIC_KEY` | 否 | 支付宝渠道；另有 `ALIPAY_GATEWAY` / `ALIPAY_SANDBOX` / `ALIPAY_MOCK` |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` | 否 | 通知邮件发信配置 |
