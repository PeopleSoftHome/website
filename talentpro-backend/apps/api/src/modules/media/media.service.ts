...
export class MediaService {
  ...
  async createSignedUrl(id: string, workspaceId?: string, ttlSeconds = 300) {
    const media = await this.repo.findOne(id);
    await this.checkWorkspaceAccess(media, workspaceId);
    const objectUrl = await this.storage.createSignedDownloadUrl(media.filename, ttlSeconds);
    if (objectUrl) {
      return { mediaId: media.id, expiresAt: Math.floor(Date.now() / 1000) + Math.min(900, Math.max(1, ttlSeconds)), url: objectUrl };
    }
  ...
  async create(data: { filename: string; originalName: string; url: string; mimeType: string; size: number; width?: number; height?: number; alt?: string; createdBy: string }) {
    return this.repo.create(data);
  }

  async createFromBuffer(data: { buffer: Buffer; filename?: string; originalName?: string; mimeType: string; alt?: string; createdBy: string }) {
    ...
    const file = { buffer: data.buffer, originalname: originalName, mimetype: data.mimeType, size: data.buffer.length } as Express.Multer.File;
    const result = await this.storage.upload(file);
    return this.repo.create({
      filename: result.filename,
      originalName: result.originalName,
  ...
  async delete(id: string, workspaceId?: string) {
    ...
    if (media) await this.storage.delete(media.filename);
    return this.repo.delete(id);
  }

  async getStats() {
    const total = await this.prisma.media.count();
    const byType = await this.prisma.media.groupBy({ by: ['mimeType'], _count: { mimeType: true }, _sum: { size: true } });
    return { total, byType };
  }
  ...
  private async checkWorkspaceAccess(media: Pick<Media, 'createdBy'>, workspaceId?: string) {
    if (!workspaceId) return;
    const creator = await this.prisma.user.findUnique({ where: { id: media.createdBy }, select: { workspaceId: true } });
    if (creator?.workspaceId !== workspaceId) throw new NotFoundException('Media file not found or no access');
  }
}
