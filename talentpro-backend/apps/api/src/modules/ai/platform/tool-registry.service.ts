...
const AUDIT_LIST_KEY = 'ai:tool:audit';
...
const DEFAULT_POLICY: GovernancePolicy = {
  ...
  enabled: true,
  maxRisk: 'medium',
  allowedTools: ['analyze_workforce', 'search_product', 'get_case', 'find_policy'],
...
export class ToolRegistryService {
  ...
  constructor(
    ...
    void this.persistPolicy(DEFAULT_POLICY);
  }
    ...
    if (keys.length) {
      ...
      for (const raw of persisted) {
        ...
        try {
          ...
          // Ignore corrupt persisted entries; live worker registrations remain available.
        }
      }
    }
  ...
  async invoke(request: ToolInvocationRequest) {
    ...
    if (definition.auditRequired || governance.allowed) {
      await this.audit({ executionId, request, definition, governance });
    }
  ...
  private async evaluateGovernance(definition: ToolDefinition, request: ToolInvocationRequest): Promise<GovernanceDecision> {
    ...
    if (!policy.enabled) {
      return { allowed: true, reasons: [], requiresApproval: false, policyVersion: policy.version };
    }

    const reasons: string[] = [];
    if (!policy.allowedTools.includes(definition.id) && !policy.allowedTools.includes('*')) {
      reasons.push(`tool ${definition.id} is not allowlisted`);
    }
    if (RISK_RANK[definition.risk] > RISK_RANK[policy.maxRisk]) {
      reasons.push(`risk ${definition.risk} exceeds maxRisk ${policy.maxRisk}`);
    }
    ...
    if (policy.maxCostUsd !== undefined && cost > policy.maxCostUsd) {
      reasons.push(`cost budget exceeded: ${cost} > ${policy.maxCostUsd}`);
    }
    ...
    const approvalReason =
      ...
      definition.risk === 'critical'
        ? 'high-risk-tool'
        : definition.sideEffects && definition.auditRequired
          ? 'external-side-effect'
          : definition.name.toLowerCase().includes('bulk')
            ? 'bulk-write'
            : null;

    if (reasons.length) {
      return { allowed: false, reasons, requiresApproval: false, policyVersion: policy.version };
    }

    const requiresApproval = Boolean(approvalReason && policy.requireHumanApprovalFor.includes(approvalReason));
    return {
      allowed: !requiresApproval || Boolean(request.context.approvalToken),
      reasons: requiresApproval ? [`human approval required: ${approvalReason}`] : [],
      requiresApproval,
  ...
  private async persistPolicy(policy: GovernancePolicy) {
    try {
      await this.redis.set(GOVERNANCE_KEY, JSON.stringify(policy));
    } catch {
      // Redis is required for production; keeping the local default avoids constructor failure in isolated unit tests.
    }
  }

  private async audit(payload: { executionId: string; request: ToolInvocationRequest; definition: ToolDefinition; governance: GovernanceDecision }) {
    if (!this.config.get<boolean>('AI_GOVERNANCE_AUDIT', true)) return;
    await this.redis.lpush(AUDIT_LIST_KEY, JSON.stringify({
      id: randomUUID(),
      at: new Date().toISOString(),
      executionId: payload.executionId,
      userId: payload.request.context.userId,
      workspaceId: payload.request.context.workspaceId || null,
      requestId: payload.request.context.requestId,
      toolId: payload.definition.id,
      version: payload.definition.version,
      governance: payload.governance,
    }));
    await this.redis.ltrim(AUDIT_LIST_KEY, 0, 9999);
  }
}
